    ...
    <div className="child-reg-page-content">
      ...
      <div className="child-reg-search-bar">
        ...
        <input 
          ...
          value={searchAllChildren} 
          onChange={(e) => setSearchAllChildren(e.target.value)} 
          disabled={isDeleting}
        />
      </div>
      <div className="child-reg-data-table-container">
            ...
            {filteredAllChildren.map((child, index) => {
              ...
              return (
                <tr key={child.id}>
                  ...
                  <td>
                    <div className="child-reg-fingerprint-summary">
                      <span className={`child-reg-fingerprint-count ${fingerCount > 0 ? 'has-fingerprints' : 'no-fingerprints'}`}>
                        {fingerStatus}
                      </span>
                      <div className="child-reg-fingerprint-dots">
                        {Array.from({ length: 10 }, (_, i) => {
                          ...
                          return (
                            <span 
                              key={fingerIndex} 
                              className={`child-reg-finger-dot ${hasFinger ? 'captured' : 'missing'}`}
                              title={`Finger ${fingerIndex}: ${hasFinger ? 'Captured' : 'Missing'}`}
                            />
                          );
                        })}
                      </div>
        ...
        {filteredAllChildren.length === 0 && (
          <div className="child-reg-no-data">
            <p>No children found. Click "Add Registration" to register a new child.</p>
          </div>
        )}
      </div>
    </div>
...
export default RenderAllChildrenList;
